...
public class SimpleVrVideoActivity extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    new Thread(
            new Runnable() {
              @Override
              public void run() {
                gvrAudioEngine.preloadSoundFile("");
                int sourceId = gvrAudioEngine.createSoundObject("mono.wav");
                gvrAudioEngine.setSoundObjectPosition(
                        sourceId, 0, 0, -20);
                gvrAudioEngine.playSound(sourceId, true/* looped playback */);
                // Preload an unspatialized sound to be played on a successful trigger on the cube.
                gvrAudioEngine.preloadSoundFile("mono.wav");
  ...
  private void setIsMuted(boolean isMuted) {
    ...
    videoWidgetView.setVolume(isMuted ? 0.0f : 100.0f);
  }
  ...
  private class ActivityEventListener extends VrVideoEventListener  {
    ...
    public void onNewFrame() {
      ...
      videoWidgetView.getHeadRotation(array);
      TextView tv = (TextView)findViewById(R.id.title);
      tv.setText("");
      String txt = "";
      if(array[1]>25 && array[1]<90)
      {
        txt += "Looking Upwards";
      }

      if(array[1]<-25 && array[1]>-90){
        txt += " Looking Downwards";
      }

      if(array[0]<-50 && array[0]>-179){
        txt += " Looking Leftwards";
      }

      if(array[0]>50 && array[0]<179){
        txt += " Looking Rightwards";
      }
      ...
      gvrAudioEngine.setHeadPosition(array[0],array[1],0);
      for(int x=0; x<array.length;x++)
      {
        Log.e("TAG",String.valueOf(array[0])+" "+String.valueOf(array[1]));
      }
    }
